...
float max_angle_test=0;

float z_vision_ratio=4.5;
...
//传送带手动控制
void trans_single_control(void);
//解算数组
...
uint32_t count_for_lift2     = 0;    //自动放矿石抬升倒计时
...
uint32_t count_for_push_vision      = 0;    //推矿计数
...
uint32_t use_absolute_flag=1;//绝对模式控制
...
sucker_switch_cmd_e last_sucker_cmd;
float calculate_distance(distance_t *distance)
{
...
//	memcpy(&custom_cmd, &video_cmd.custom_robot_data, 19);//专门针对自定义控制器解码
    ...
    custom_cmd.vw = (video_cmd.custom_robot_data.data[24] << 8) | video_cmd.custom_robot_data.data[23]; // data[4] 是低字节，data[5] 是高字节
	  ...
	  custom_cmd.push_flag=video_cmd.custom_robot_data.data[25];
...
//		chassis_vision_ctrl_adjust(&Chassis_CMD_data,&minipc);
		arm_vision_ctrl_final(&ARM_CMD_data, &minipc);
		
		
				    if(f_flag){
            if (count_for_push_vision == 0) {
                target_height = height;
            }
            //球坐标->笛卡尔坐标
            float rho    = 1; //默认长度
            float theta  =    finesse_motor.para.pos+ max_motor.para.pos + min_motor.para.pos;;
...
//            float chassis_y   = 102;
            ...
            count_for_push_vision++;
						}
						else{
							count_for_push_vision=0;	
						}
						
						
			  if (q_flag || e_flag) {
            ARM_CMD_data.roll_mode = ROLL_ANGLE_MODE;//角度模式 增量控制
					  ARM_CMD_data.roll_angle = ((0.5f *e_flag) - (0.5f * q_flag)) + roll_real;//增量式控制
        } else {
            ARM_CMD_data.roll_mode = ROLL_KEEP_MODE;//保持
        }
		
		
//		if(minipc.no_data_time>0.5)
//		{
...
//	if(custom_cmd.sucker_cmd!=last_sucker_cmd)
//	{
...
//	{ARM_CMD_data.sucker_mode = SUCKER_OFF;	}
//	}
//	last_sucker_cmd=custom_cmd.sucker_cmd;

	
	  if_solve_flag=0;//不需要解算
	...
	else if(custom_cmd.push_flag==1)
	{
		        ARM_CMD_data.sucker_mode = SUCKER_ON;
...
//            float chassis_x   = 72;
//            float chassis_y   = 102;
						float z_base_down =adjust8;//0.27
            float z_base_up   = 0.59f;//59
            float chassis_x   = 59;//38 76
            float chassis_y   = 80;//50 100
...
//            ARM_CMD_data.auto_mode=GET_SILVER;
            ...
            yaw_absolute      = 0;
				 ...
				 if (r_counter->shift_press_count % 2 == 1) {//自动侧面放矿
//            ARM_CMD_data.auto_mode=GET_SILVER;
					auto_operate_mode = 4;
            yaw_absolute      = 0;
        }
	...
	AUTO_STATE=HANDLE;
	 /******************手动部分********************/
        ...
        if (rc_ctrl.rc.ch[0] > 100) {//上下
            rc_mode_xy[0] += (rc_ctrl.rc.ch[3] ) / 420;
        }
        if (rc_ctrl.rc.ch[0] < -100) {
            rc_mode_xy[0] += (rc_ctrl.rc.ch[3] ) / 420;
        }
        if (rc_ctrl.rc.ch[1] > 100) {
            rc_mode_xy[1] -= (rc_ctrl.rc.ch[1]) / 420;
        }
        if (rc_ctrl.rc.ch[1] < -100) {
            rc_mode_xy[1] -= (rc_ctrl.rc.ch[1]) / 420;
        }
        ...
        if (q_flag || e_flag) {
            ARM_CMD_data.roll_mode = ROLL_ANGLE_MODE;//角度模式 增量控制
					  ARM_CMD_data.roll_angle = ((0.1f *e_flag) - (0.1f * q_flag)) + roll_real;//增量式控制
        } else {
            ARM_CMD_data.roll_mode = ROLL_KEEP_MODE;//保持
        }
        ...
        if (NEW_Remote0.custom_right) {//左拨杆向上
            target_lift_speed = 300;
				...
				if ((NEW_Remote0.custom_right==1&&NEW_Remote0.custom_right==1)){//左拨杆中间
            target_lift_speed = 0;
        }
        ...
        if (press_left && (!press_right)) {
            ...
            target_lift_speed=target_lift_speed-10.f;
					...
					if(!(NEW_Remote0.custom_right==1||NEW_Remote0.custom_right==1))
            target_lift_speed =0;
        }
			}
...
void normally_set_all_data(void)
{
	  ARM_CMD_data.maximal_arm_angle = -0.27f + target_angle1;//1.05
	...
	
{
        ...
        if (z_counter->shift_press_count % 3 == 2) { //这个模式不会长时间持续
						AUTO_STATE=LOCK_ARM;
            lift_height_cmd(125, &target_lift_speed);
            rc_mode_xy[0]     = 330;
            rc_mode_xy[1]     = 0;
            target_angle4     = 0;
            yaw_absolute      = 0;
...
//其他模式清零
        ...
        if (r_counter->shift_press_count % 2 == 0) 
        {
            auto_operate_mode = 0;
            count_for_drop    = 0;
				   	count_for_lift2   = 0;
...
//        target_angle4               = -PI / 2;
        ...
        if (height < 200) //在危险高度以下
        {
            ...
            if (backback_step == 0)
            {
                lift_height_cmd(660, &target_lift_speed); 
                rc_mode_xy[0] = 250;
                rc_mode_xy[1] += 4;
                if (rc_mode_xy[1] >= 400)
                                                                {
            ...
            if (backback_step == 1) {//半收手
                lift_height_cmd(590, &target_lift_speed); 

                target_angle1     = 0;
                target_angle2     = 2.22921f;

                use_absolute_flag=0;//不要绝对控制

                target_angle3     =-0.4001;//488
                ...
                if (fabs(min_motor.para.pos- 2.22921f) < 0.40) {
                    backback_step = 2;
                }
                if_solve_flag = 0; //机械臂不解算
            }
            ...
            if (backback_step == 2) {//预备放矿高度
							
                lift_height_cmd(290, &target_lift_speed); 
                target_angle1     = -0.163462639;//42956f
                target_angle2     = 2.68959332;//15921f
                target_angle3     =0.749790192;//488
						   	target_angle4     =0;
                 ARM_CMD_data.trans_mode = STOP;

                if (fabs(min_motor.para.pos - 2.68959332) < 0.15) {
                    backback_step = 3;
                }
                if_solve_flag = 0; //机械臂不解算
            }
            ...
            if (backback_step == 3) {//下放高度
                ARM_CMD_data.sucker_mode = SUCKER_OFF;
                lift_height_cmd(220, &target_lift_speed); //350
                target_angle1     = -0.163462639;//42956f
                target_angle2     = 2.68959332;//15921f
                target_angle3     =0.749790192;//488
						   	target_angle4     =0;
                ...
                if_solve_flag = 0; //机械臂不解算
						...
						if (backback_step == 3.5) {//这里机械臂角度抽手
                ARM_CMD_data.sucker_mode = SUCKER_OFF;
                lift_height_cmd(590, &target_lift_speed); //350
               target_angle1     = -0.163462639;//42956f
                target_angle2     = 2.68959332;//15921f
                target_angle3     =0.749790192;//488
						   	target_angle4     =0;
            ...
            if (backback_step == 4) {
                ...
                if ((fabs(rc_mode_xy[0] - temp_xy[0]) < 60) && (fabs(rc_mode_xy[1] - temp_xy[1]) < 60)) {//坐标差小于一定程度
                    //回到手动控制模式
                    r_counter->shift_press_count = 0;
                    backback_step = 0; //步进标志位清零
                    r_counter->shift_press_count += 2;
                 if (r_counter->shift_press_count % 2 == 0) 
                 {
                 ARM_CMD_data.sucker_mode                             = SUCKER_OFF;
                 }
                }
                if_solve_flag = 1; //机械臂解算B
            }
        }
	...
	AUTO_STATE=GET_SILVER;
	 //开吸盘
        ...
        if (silver_mode_step == 2) {
            ...
            if (backback_step == 1) {
                lift_height_cmd(590, &target_lift_speed);
                target_angle1     = 0.95956f;
                target_angle2     = 2.66921f;
                if (fabs(min_motor.para.pos - 2.66921f) < 0.15) {
                    backback_step = 2;
                }
                if_solve_flag = 0; //机械臂不解算
            }
            if (backback_step == 2) {
                lift_height_cmd(590, &target_lift_speed); 
                target_angle1     = 1.42729f;//42956f
                target_angle2     = 2.33710f;//15921f
                if (fabs(min_motor.para.pos- 2.33710f) < 0.15) {
                    backback_step = 3;
                }
                if_solve_flag = 0; //机械臂不解算
            }
            ...
            if (backback_step == 4) {
                ...
                rc_mode_xy[1]     = 200;
							  ...
							  use_absolute_flag=1;//不要绝对控制
                float temp_xy[2]  = {0, 0};
                scara_forward_kinematics(max_motor.para.pos, min_motor.para.pos, ARMLENGHT1, ARMLENGHT2, temp_xy);
                if ((fabs(rc_mode_xy[0] - temp_xy[0]) < 25) && (fabs(rc_mode_xy[1] - temp_xy[1]) < 25)) {
                    backback_step            = 0; //步进标志位清零
                    ARM_CMD_data.sucker_mode = SUCKER_ON;
        ...
        if (silver_mode_step == 3) {
            ...
            yaw_absolute      = 0;
            target_angle4     = -PI / 2;
            lift_height_cmd(0, &target_lift_speed); 
            count_for_step3++;
            if (count_for_step3 > 700) {
                silver_mode_step = 4;
            }
        }
...
void push_block(void)
{
        ...
        if (f_flag) {
            ...
            float x_base      = 0.5f;
            float y_base      = 0.5f;
            float z_base_down = 0.32f;
            float z_base_up   = 0.108f;//59
            float chassis_x   = 90;
            float chassis_y   = 110;
...
//        target_angle4               = -PI / 2 - 0.14;
//        ARM_CMD_data.sucker_mode    = SUCKER_ON;
//        if (height < 230) //在危险高度以下
//        {
//            target_lift_speed = 500;
//        } else { //可以开始收回
//            if (backget_step == 0) {
...
//            }
//            if (backget_step == 1) {
//							  lift_height_cmd(400, &target_lift_speed); 
//                target_angle1     = 1.42956f;
//                target_angle2     = 2.11921f;
//                if (fabs(min_motor.para.pos - 2.11921f) < 0.15) {
//                    backget_step = 2;
//                }
//                if_solve_flag = 0; //机械臂不解算
//            }
//            if (backget_step == 2) {
//							  lift_height_cmd(260, &target_lift_speed); 
//                
//                target_angle1     = 1.42956f;
//                target_angle2     = 2.11921f;
//                count_for_suck++;
//                if (count_for_suck > 400) {
//                    backget_step = 3;
//                }
//                if_solve_flag = 0; //机械臂不解算
//            }
...
//            if (backback_step == 3) {
//                ARM_CMD_data.sucker_mode = SUCKER_OFF;
//                lift_height_cmd(375, &target_lift_speed); 
//                target_angle1            = 1.42956f;
//                target_angle2            = 2.15921f;
//							  
//							
...
{      
        ...
        if (height < 230) //在危险高度以下
        {
            ...
            if (backget_step == 1) {
                use_absolute_flag=0;//不要绝对控制
                lift_height_cmd(430, &target_lift_speed); 
                target_angle1     = 0.95956f;
                target_angle2     = 2.66921f;

                if (fabs(min_motor.para.pos - 2.66921f) < 0.40) {
                    backget_step = 2;
                }
                if_solve_flag = 0; //机械臂不解算
            }
            if (backget_step == 2) {
                lift_height_cmd(295, &target_lift_speed); //280

                target_angle1     = 1.42956f;
                target_angle2     = 2.31921f;
                count_for_suck++;
                if (count_for_suck > 300) {
                    backget_step = 3;
                }
                if_solve_flag = 0; //机械臂不解算
            }
...
//        target_angle4               = -PI / 2;
        ...
        if (height < 250) //在危险高度以下
        {
            ...
            if (backback_step == 3) {
                ARM_CMD_data.sucker_mode = SUCKER_OFF;
                lift_height_cmd(360, &target_lift_speed); //350
                target_angle1            = 1.42729f;
                target_angle2            = 2.33710f;
...
//        }
				...
				if(ctrl_flag&&d_flag&& (!a_flag))
				{			
           if (Chassis_CMD_data.vx > 3800 * speed_scale) {//2000 4000
            Chassis_CMD_data.vx = 3800 * speed_scale;
           }
			  }
				if((!ctrl_flag)&&d_flag&& (!a_flag))
				{
				
           if (Chassis_CMD_data.vx > 2000 * speed_scale) {//2000
            Chassis_CMD_data.vx = 2000 * speed_scale;
           }
			  }
				
				
				if(ctrl_flag&&a_flag&& (!d_flag))
				{			
           if (Chassis_CMD_data.vx < -3800 * speed_scale) {//2000 4000
            Chassis_CMD_data.vx = -3800 * speed_scale;
           }
        ...
        if (target_forward < -44) {
            target_forward = -44;
				...
				if(ctrl_flag&&w_flag&& (!s_flag))
				{			
           if (Chassis_CMD_data.vy > 5200 * speed_scale) {//2000 4000
            Chassis_CMD_data.vy = 5200 * speed_scale;
           }
			  }
				if((!ctrl_flag)&&w_flag&& (!s_flag))
				{
				
           if (Chassis_CMD_data.vy > 3400 * speed_scale) {//2000
            Chassis_CMD_data.vy = 3400 * speed_scale;
           }
			  }
...
//        }
				...
				if(ctrl_flag&&s_flag&& (!w_flag))
				{			
           if (Chassis_CMD_data.vy < -3800 * speed_scale) {//2000 4000
            Chassis_CMD_data.vy = -3800 * speed_scale;
           }
			  }
				if((!ctrl_flag)&&s_flag&& (!w_flag))
				{
				
           if (Chassis_CMD_data.vy < -2000 * speed_scale) {//2000
            Chassis_CMD_data.vy = -2000 * speed_scale;
           }
			  }
...
void chassis_vision_ctrl_adjust(Chassis_CMD_data_t *chassis_cmd,minipc_t *minipc)
{
	if(minipc->minipc2mcu.x_erro<-limit)minipc->minipc2mcu.x_erro=-limit;
	if(minipc->minipc2mcu.x_erro>limit)minipc->minipc2mcu.x_erro=limit;
	if(minipc->minipc2mcu.y_erro<-limit)minipc->minipc2mcu.y_erro=-limit;
	if(minipc->minipc2mcu.y_erro>limit)minipc->minipc2mcu.y_erro=limit;	
//	chassis_cmd->vx=-PID_calc(&x_vision_ctrl_pid,minipc->minipc2mcu.x_erro,0);
//  chassis_cmd->vy=-PID_calc(&y_vision_ctrl_pid,minipc->minipc2mcu.y_erro,0);此处做出修改
	if(chassis_cmd->vx!=0||chassis_cmd->vy!=0)
	{
		minipc->mcu2minipc.if_chassis_move=1;
	}
	if(chassis_cmd->vx==0||chassis_cmd->vy==0)
	{
		minipc->mcu2minipc.if_chassis_move=0;
	}
	
	float speed_scale=0.14;
	
				if (chassis_cmd->vx > 500 * speed_scale) {
            chassis_cmd->vx = 500 * speed_scale;
        }
        if (chassis_cmd->vx < -500 * speed_scale) {
            chassis_cmd->vx = -500 * speed_scale;
        }
				
				if (chassis_cmd->vy > 500 * speed_scale) {
            chassis_cmd->vy = 500 * speed_scale;
        }
        if (chassis_cmd->vy < -500 * speed_scale) {
            chassis_cmd->vy = -500 * speed_scale;
        }
...
void trans_single_control(void)
{
	if(z_counter->single_press_count%3==0)
	{
		ARM_CMD_data.trans_mode=STOP;
	}
	if(z_counter->single_press_count%3==1)
	{
		ARM_CMD_data.trans_mode=GET;
	}
	if(z_counter->single_press_count%3==2)
	{
		ARM_CMD_data.trans_mode=OUTPUT;
	}
